...
description: When an assistant calls a tool on your custom MCP server, Use Brian attaches server-resolved X-UseBrian-Actor-* headers so your server can authorize per end user.
tags: [api, connector-identity]
canonical: https://usebrian.ai/docs/api/connector-identity
---

> Human-readable version: https://usebrian.ai/docs/api/connector-identity
...
The identity is resolved from the authenticated session, which the model never reads, and it is attached to the request after the model has produced its tool call. The model has no path to read, set, or alter it. User-configured headers also cannot use the `X-UseBrian-` prefix, so the namespace cannot be spoofed from configuration either.

## Other headers

Beyond identity, a connector owner can attach fixed preflight headers (for tenant, routing, or tracing) and the connector's own auth header. Identity uses the reserved `X-UseBrian-` namespace and always takes precedence.
...
- Use `X-UseBrian-User-Id` as the primary key for per-user storage; treat `X-UseBrian-Actor-Id` and `X-UseBrian-Actor-Email` as changeable.
